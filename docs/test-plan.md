...
|---|---|---|---|
| T-SSO-1 | Unauthenticated browser → `chat.lab.gooptimal.io` | 302 to `*.cloudflareaccess.com`; Okta login screen; MFA prompt; lands in Open WebUI as `ryan@…` | Screenshots: redirect, Okta, MFA, Open WebUI signed-in |
| T-SSO-2 | User in `lab-users` only → `gateway.lab.gooptimal.io` | Cloudflare Access **Block** page naming the gateway policy | Screenshot of block page |
...
**T-AUDIT-1.** Sign in to `chat.lab.gooptimal.io` as `ryan@…`, complete Okta MFA,
...
| 2 | **Cloudflare Access log** | `Allow` for the same email on `chat.lab.gooptimal.io`, **policy = `allow-lab-users`** | Zero Trust → Logs → Access |
