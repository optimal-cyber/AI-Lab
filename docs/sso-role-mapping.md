...
A user in `lab-users` opens `chat.lab.gooptimal.io`, completes Okta MFA, and
...
2. **Cloudflare Access log** (Zero Trust → Logs → Access) — an `Allow` event for
   `ryan@…` on `chat.lab.gooptimal.io`, **policy matched = `allow-lab-users`**,
...
For the strict path, swap step 2 for an `Allow` on `gateway.lab.gooptimal.io`
